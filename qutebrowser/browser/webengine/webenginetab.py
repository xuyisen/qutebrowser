        ...
        def wrapped_callback(cb_arg):
            ...
            if self._pending_searches > 0:
                ...
                log.webview.debug(
                    ...
                    f"{self._pending_searches} pending searches"
                )
                return
    ...
    def search(
        ...
        *,
        ...
        if self.text == text and self.search_displayed:
            log.webview.debug(
                f"Ignoring duplicate search request for {text}, but resetting flags"
            )
...
class WebEngineCaret(browsertab.AbstractCaret):
    ...
    def _follow_selected_cb(self, js_elem, tab):
        ...
        if elem.is_link():
            log.webview.debug(
                "Found link in selection, clicking. ClickTarget "
                f"{click_type}, elem {elem}"
            )
            try:
    ...
    def _toggle_sel_translate(self, state_str):
        if self._mode_manager.mode != usertypes.KeyMode.caret:
            ...
            log.misc.debug(
                f"Ignoring caret selection callback in {self._mode_manager.mode}"
            )
            return
    ...
    def _update_pos(self, pos):
        ...
        if scrollable_x == 0:
            ...
            try:
                ...
                log.misc.debug(f"contents_size.width(): {contents_size.width()}")
                log.misc.debug(f"self._widget.width(): {self._widget.width()}")
                log.misc.debug(f"scrollable_x: {scrollable_x}")
                log.misc.debug(f"pos.x(): {pos.x()}")
                raise
        ...
        if scrollable_y == 0:
            ...
            try:
                ...
                log.misc.debug(f"contents_size.height(): {contents_size.height()}")
                log.misc.debug(f"self._widget.height(): {self._widget.height()}")
                log.misc.debug(f"scrollable_y: {scrollable_y}")
                log.misc.debug(f"pos.y(): {pos.y()}")
                raise
...
class WebEngineElements(browsertab.AbstractElements):
    ...
    def _js_cb_single(self, callback, js_elem):
        ...
        log.webview.debug(f"Got element from JS: {debug_str}")
...
class _WebEnginePermissions(QObject):
    ...
    def _on_feature_permission_requested(self, url, feature):
        ...
        if not url.isValid():
            log.webview.warning(
                f"Ignoring feature permission {permission_str} for invalid URL {url}"
            )
            deny_permission()
            return

        if feature not in self._options:
            log.webview.error(f"Unhandled feature permission {permission_str}")
            deny_permission()
            return
    ...
    def _on_quota_requested(self, request):
        ...
        shared.feature_permission(
            ...
            msg=f"use {size} of persistent storage",
    ...
    def _on_register_protocol_handler_requested(self, request):
        shared.feature_permission(
            ...
            msg=f"open all {request.scheme()} links",
            yes_action=request.accept,
            no_action=request.reject,
            abort_on=[self._tab.abort_questions],
            blocking=True,
    ...
    def _remove_all_greasemonkey_scripts(self):
        ...
        for script in page_scripts.toList():
            if script.name().startswith("GM-"):
                log.greasemonkey.debug(f"Removing script: {script.name()}")
...
class WebEngineTab(browsertab.AbstractTab):
        ...
        if world is None:
            ...
            if not 0 <= world_id <= qtutils.MAX_WORLD_ID:
                raise browsertab.WebTabError(
                    f"World ID should be between 0 and {qtutils.MAX_WORLD_ID}"
                )
        else:
    ...
    def _show_error_page(self, url, error):
        ...
        log.misc.debug(f"Showing error page for {error}")
        url_string = url.toDisplayString()
        error_page = jinja.render(
            "error.html",
            title=f"Error loading page: {url_string}",
            url=url_string,
            error=error,
        )
    ...
    def _on_proxy_authentication_required(self, url, authenticator, proxy_host):
        ...
        msg = (
            f"<b>{html_utils.escape(proxy_host)}</b> requires a username and password."
        )
        urlstr = url.toString(
    ...
    def _on_authentication_required(self, url, authenticator):
        log.network.debug(
            f"Authentication requested for {url.toDisplayString()}, netrc_used {self.data.netrc_used}"
        )
    ...
    def _on_renderer_process_pid_changed(self, pid):
        log.webview.debug(f"Renderer process PID for tab {self.tab_id}: {pid}")
    ...
    def _error_page_workaround(self, js_enabled, html):
        ...
        error = match.group(1)
        log.webview.error(f"Load error: {error}")
    ...
    def _on_ssl_errors(self, error):
        ...
        log.network.debug(f"Certificate error: {error}")
        log.network.debug(f"First party URL: {first_party_url}")
        ...
        log.network.debug(
            f"ignore {error.ignore}, URL {url}, requested {self.url(requested=True)}"
        )
    ...
    def _on_select_client_certificate(self, selection):
        ...
        present = message.ask(
            title=f"Present client certificate to {urlstr}?",
            text=text,
