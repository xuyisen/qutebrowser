...
def wait_ssl_page_finished_loading(quteproc, ssl_server):
    quteproc.wait_for_load_finished(
        "/", port=ssl_server.port, https=True, load_status="warn"
    )
...
def fresh_instance(quteproc):
    ...
    #
    # Qt 6.10+ accidentally persists some permissions;
...
def test_certificate_error_load_status(request, quteproc, ssl_server):
    ...
    for i in range(2):
        quteproc.open_path(
            "/", port=ssl_server.port, https=True, wait=False, new_tab=True
        )
