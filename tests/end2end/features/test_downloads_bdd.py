...
def wait_for_download_finished_name(quteproc, name):
    quteproc.wait_for(category="downloads", message=f"Download {name} finished")
...
def default_open_dispatcher_python(quteproc, tmpdir):
    cmd = f'{shlex.quote(sys.executable)} -c "import sys; print(sys.argv[1])"'
...
def download_open(quteproc):
    cmd = f'{shlex.quote(sys.executable)} -c "import sys; print(sys.argv[1])"'
    quteproc.send_cmd(f":download-open {cmd}")
...
def download_open_with_prompt(quteproc):
    ...
    quteproc.send_cmd(f":prompt-open-download {cmd}")
