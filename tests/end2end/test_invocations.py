...
from qutebrowser.qt.core import QPoint, QProcess
from qutebrowser.utils import qtutils, utils, version

ascii_locale = pytest.mark.skipif(
    sys.hexversion >= 0x03070000,
    reason="Python >= 3.7 doesn't force ASCII locale with LC_ALL=C",
)
...
def test_downloads_with_ascii_locale(request, server, tmp_path, quteproc_new):
    ...
    url = f"http://localhost:{server.port}/data/downloads/ä-issue908.bin"
    quteproc_new.send_cmd(f":download {url}")
    quteproc_new.wait_for(
        category="downloads", message="Download ?-issue908.bin finished"
    )

    # Test :prompt-open-download
    quteproc_new.set_setting("downloads.location.prompt", "true")
    quteproc_new.send_cmd(f":download {url}")
    quteproc_new.send_cmd(f':prompt-open-download "{sys.executable}" -c pass')
    quteproc_new.wait_for(
        category="downloads", message="Download ä-issue908.bin finished"
    )
...
def test_open_with_ascii_locale(request, server, tmp_path, quteproc_new, url):
    ...
    quteproc_new.send_cmd(f":open {url}")

    if not request.config.webengine:
        line = quteproc_new.wait_for(
            message="Error while loading *: Error opening /*: No such file or directory"
        )
        line.expected = True

    quteproc_new.wait_for(
        message="load status for <* tab_id=* "
...
def test_open_command_line_with_ascii_locale(request, server, tmp_path, quteproc_new):
    ...
    quteproc_new.wait_for(
        message="load status for <* tab_id=* url='*/f*.html'>: LoadStatus.error"
    )
...
@pytest.mark.linux
def test_misconfigured_user_dirs(
    request, server, temp_basedir_env, tmp_path, quteproc_new
):
    ...
    quteproc_new.set_setting("downloads.location.prompt", "false")
    url = f"http://localhost:{server.port}/data/downloads/download.bin"
    quteproc_new.send_cmd(f":download {url}")
    line = quteproc_new.wait_for(
    ...
    line.expected = True
    quteproc_new.wait_for(
        category="downloads", message="Download download.bin finished"
    )
...
def test_no_loglines(request, quteproc_new):
    ...
    quteproc_new.start(
        args=["--temp-basedir", "--loglines=0"] + _base_args(request.config)
    )
...
def test_webengine_download_suffix(request, quteproc_new, tmp_path):
    ...
    (tmp_path / "user-dirs.dirs").write_text(f"XDG_DOWNLOAD_DIR={download_dir}")
...
def test_qute_settings_persistence(short_tmpdir, request, quteproc_new):
    ...
    quteproc_new.wait_for(
        category="config", message="Config option changed: search.ignore_case = always"
    )
...
def test_preferred_colorscheme_with_dark_mode(
    ...
    if webengine_versions.webengine == utils.VersionNumber(5, 15, 2):
        ...
        if webengine_versions.webengine >= utils.VersionNumber(6):
            expected_color = (
                testutils.Color(148, 146, 148)
                if IS_ARM
                else testutils.Color(144, 144, 144)
            )
        else:
            expected_color = (
                testutils.Color(123, 125, 123)
                if IS_ARM
        ...
        expected_color = (
            testutils.Color(33, 32, 33) if IS_ARM else testutils.Color(34, 34, 34)
        )  # dark website color
        xfail = False
...
def test_dark_mode(
    webengine_versions, quteproc_new, request, filename, algorithm, colors
):
    if not request.config.webengine:
        pytest.skip("Skipped with QtWebKit")
...
def test_unavailable_backend(request, quteproc_new):
    ...
    line = quteproc_new.wait_for(
        message=(
            "*qutebrowser tried to start with the Qt* backend but failed "
